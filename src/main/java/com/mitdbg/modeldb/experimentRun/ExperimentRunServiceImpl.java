...
import com.mitdbg.modeldb.ExperimentRunStoreArtifact;
import com.mitdbg.modeldb.ExperimentRunStoreArtifact.StoreArtifact;
import com.mitdbg.modeldb.ExperimentRunStoreArtifact.StoreArtifact.Response;
...
public class ExperimentRunServiceImpl extends ExperimentRunServiceImplBase {
  ...
  @Override
  public void storeArtifact(StoreArtifact request, StreamObserver<Response> responseObserver) {

    try {

      if (request.getExperimentRunId().isEmpty()
          || request.getFileKey().isEmpty()
          || request.getClientFilepath().isEmpty()) {
        String errorMessage =
            "ExperimentRun ID OR file key OR file path is not found in StoreArtifact request.";
        LOGGER.log(Level.WARNING, errorMessage);
        Status status =
            Status.newBuilder()
                .setCode(Code.INVALID_ARGUMENT_VALUE)
                .setMessage(errorMessage)
                .addDetails(Any.pack(StoreArtifact.Response.getDefaultInstance()))
                .build();
        throw StatusProto.toStatusRuntimeException(status);
      }

      String clientKey = request.getFileKey();

      String cloudStorageKey =
          artifactStoreDAO.getCloudStorageKey(
              app.getExperimentRunEntity(), request.getExperimentRunId(), clientKey);
      if (cloudStorageKey != null) {
        String errorMessage = "Given file key is already used, Please try with other key.";
        LOGGER.log(Level.WARNING, errorMessage);
        Status status =
            Status.newBuilder().setCode(Code.NOT_FOUND_VALUE).setMessage(errorMessage).build();
        throw StatusProto.toStatusRuntimeException(status);
      ...
      com.mitdbg.artifactstore.StoreArtifact storeArtifactRequest =
          com.mitdbg.artifactstore.StoreArtifact.newBuilder()
              .setKey(clientKey)
              .setClientFilePath(request.getClientFilepath())
              .build();

      ArtifactStoreBlockingStub artifactStoreStub =
          ArtifactStoreGrpc.newBlockingStub(app.getArtifactServerChannel());
      com.mitdbg.artifactstore.StoreArtifact.Response responseStatus =
          artifactStoreStub.storeArtifact(storeArtifactRequest);

      String cloudStorageFileKey = responseStatus.getCloudFileKey();
      String cloudStorageFilePath = responseStatus.getCloudFilePath();

      Artifact artifact =
          Artifact.newBuilder().setKey(clientKey).setPath(request.getClientFilepath()).build();

      ExperimentRun experimentRun =
          experimentRunDAO.storeCloudArtifact(request.getExperimentRunId(), artifact);

      artifactStoreDAO.storeKeyMapping(
          app.getExperimentRunEntity(),
      ...
      responseObserver.onNext(
          StoreArtifact.Response.newBuilder().setExperimentRun(experimentRun).build());
      responseObserver.onCompleted();
  ...
  @Override
  public void storeArtifactWithStream(
      StoreArtifactWithStream request,
      StreamObserver<StoreArtifactWithStream.Response> responseObserver) {

    try {

      if (request.getExperimentRunId().isEmpty()
          || request.getFileKey().isEmpty()
          || request.getClientFile().isEmpty()) {
        String errorMessage =
            "ExperimentRun ID OR file key OR file is not found in StoreStreamArtifact request.";
        LOGGER.log(Level.WARNING, errorMessage);
        Status status =
            Status.newBuilder()
                .setCode(Code.INVALID_ARGUMENT_VALUE)
                .setMessage(errorMessage)
                .addDetails(Any.pack(StoreArtifactWithStream.Response.getDefaultInstance()))
                .build();
        throw StatusProto.toStatusRuntimeException(status);
      }
      ...
      com.mitdbg.artifactstore.StoreArtifactWithStream storeArtifactRequest =
          com.mitdbg.artifactstore.StoreArtifactWithStream.newBuilder()
              .setKey(clientKey)
              .setClientFile(request.getClientFile())
              .build();

      ArtifactStoreBlockingStub artifactStoreStub =
          ArtifactStoreGrpc.newBlockingStub(app.getArtifactServerChannel());
      com.mitdbg.artifactstore.StoreArtifactWithStream.Response responseStatus =
          artifactStoreStub.storeArtifactWithStream(storeArtifactRequest);
      ...
      Artifact artifact =
          Artifact.newBuilder()
              .setKey(clientKey)
              .setPath(request.getClientFile().toString())
      ...
      responseObserver.onNext(
          StoreArtifactWithStream.Response.newBuilder().setExperimentRun(experimentRun).build());
      responseObserver.onCompleted();
  ...
  @Override
  public void getStoreArtifact(
      ExperimentRunStoreArtifact.GetArtifact request,
      StreamObserver<ExperimentRunStoreArtifact.GetArtifact.Response> responseObserver) {
    try {

      if (request.getExperimentRunId().isEmpty() || request.getKey().isEmpty()) {
        String errorMessage =
            "ExperimentRun ID OR file key is not found in ExperimentRunStoreArtifact.GetArtifact request.";
        LOGGER.log(Level.WARNING, errorMessage);
        Status status =
            Status.newBuilder()
                .setCode(Code.INVALID_ARGUMENT_VALUE)
                .setMessage(errorMessage)
                .addDetails(
                    Any.pack(ExperimentRunStoreArtifact.GetArtifact.Response.getDefaultInstance()))
                .build();
        throw StatusProto.toStatusRuntimeException(status);
      }
      ...
      String cloudStorageKey =
          artifactStoreDAO.getCloudStorageKey(
              app.getExperimentRunEntity(), request.getExperimentRunId(), request.getKey());
      if (cloudStorageKey == null) {
        String errorMessage =
            "Given client key is invalid OR client key not present in ArtifactStoreMapping Entity";
        LOGGER.log(Level.WARNING, errorMessage);
        Status status =
            Status.newBuilder().setCode(Code.NOT_FOUND_VALUE).setMessage(errorMessage).build();
        throw StatusProto.toStatusRuntimeException(status);
      ...
      GetArtifact getArtifactRequest = GetArtifact.newBuilder().setKey(cloudStorageKey).build();
      GetArtifact.Response response = artifactStoreStub.getArtifact(getArtifactRequest);

      responseObserver.onNext(
          ExperimentRunStoreArtifact.GetArtifact.Response.newBuilder()
              .setContents(response.getContents())
              .build());
      responseObserver.onCompleted();
  ...
  @Override
  public void deleteStoreArtifact(
      ExperimentRunStoreArtifact.DeleteArtifact request,
      StreamObserver<ExperimentRunStoreArtifact.DeleteArtifact.Response> responseObserver) {

    try {

      if (request.getExperimentRunId().isEmpty() || request.getKey().isEmpty()) {
        String errorMessage =
            "ExperimentRun ID OR file key is not found in ExperimentRunStoreArtifact.DeleteArtifact request.";
        LOGGER.log(Level.WARNING, errorMessage);
        Status status =
            Status.newBuilder()
                .setCode(Code.INVALID_ARGUMENT_VALUE)
                ...
                .addDetails(
                    Any.pack(
                        ExperimentRunStoreArtifact.DeleteArtifact.Response.getDefaultInstance()))
                .build();
        throw StatusProto.toStatusRuntimeException(status);
      }
      ...
      LOGGER.log(Level.INFO, "client key & cloud storage key mapping found");

      DeleteArtifact deleteArtifactRequest =
          DeleteArtifact.newBuilder().setKey(cloudStorageKey).build();
      DeleteArtifact.Response response = artifactStoreStub.deleteArtifact(deleteArtifactRequest);

      Boolean status = false;
      if (response.getStatus()) {
        status = artifactStoreDAO.deleteStoreKeyMapping(cloudStorageKey);
        experimentRunDAO.deleteCloudArtifact(request.getExperimentRunId(), request.getKey());
      }

      responseObserver.onNext(
          ExperimentRunStoreArtifact.DeleteArtifact.Response.newBuilder()
              .setStatus(status)
              .build());
      responseObserver.onCompleted();
      ...
      Status status =
          Status.newBuilder()
              ...
              .setMessage(e.getMessage())
              .addDetails(
                  Any.pack(ExperimentRunStoreArtifact.DeleteArtifact.Response.getDefaultInstance()))
              .build();
      responseObserver.onError(StatusProto.toStatusRuntimeException(status));
    }
  }
